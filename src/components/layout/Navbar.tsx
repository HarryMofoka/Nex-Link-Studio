      ...
      >
        ...
        <div className="px-8 py-10 flex-1 overflow-y-auto no-scrollbar">
          <nav className="flex flex-col gap-8 text-2xl font-medium text-neutral-900 dark:text-white">
            <Link
              href="/"
              className={`transform transition-all duration-500 delay-[50ms] ${pathname === "/" ? "text-purple-700 dark:text-purple-400" : "hover:text-purple-700 dark:hover:text-purple-500"} ${isMobileMenuOpen ? "translate-y-0 opacity-100" : "translate-y-4 opacity-0"}`}
            >
              Home
            </Link>
            {NAV_LINKS.map((link, idx) => (
              <div key={link.href}>
                <div className={`w-full h-px bg-black/5 dark:bg-white/5 transition-opacity duration-500 delay-[${(idx + 1) * 100}ms] ${isMobileMenuOpen ? "opacity-100" : "opacity-0"} mb-8`} />
                <Link
                  href={link.href}
                  className={`transform transition-all duration-500 delay-[${(idx + 1) * 100 + 50}ms] ${
                    pathname === link.href ? "text-purple-700 dark:text-purple-400" : "hover:text-purple-700 dark:hover:text-purple-500"
